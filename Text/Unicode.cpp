...
//
//  The Unicode combining and square data portions of
//  this file are from:
//  https://github.com/ThomasDickey/uniset-snapshots
//  and were generated from the uniset utility developed by
...
std::array<Interval, 339> const combining =
{{
...
std::array<Interval, 122> const square =
{{
...
template <size_t N>
bool searchIntervals(
  std::array<Interval, N> const & set,
  uint32_t value)
{
  unsigned int left = 0;
  ...
  while (left < right)
  {
    ...
    if (value < set[middle].mFirst)
    {
      right = middle;
    }
    else if (value > set[middle].mLast)
    {
      left = middle + 1;
    }
    else
...
bool TUCUT::Text::isControlCharacter (uint32_t character)
{
  if (character <= 0x19 ||
...
bool TUCUT::Text::isCombiningCharacter (uint32_t character)
{
  return searchIntervals(combining, character);
}

bool TUCUT::Text::isBaseCharacter (uint32_t character)
{
...
bool TUCUT::Text::isSquareCharacter (uint32_t character)
{
  return searchIntervals(square, character);
}
